...
def page(day: str, tweets: list, topics: list) -> str:
    ...
    topic_items = "".join(
        ...
        f"{html.escape(str(t.get('title', t.get('topic', ''))))}</li>" for t in topics[:12]
    )
...
def main() -> int:
    ...
    if trends_f.exists():
        try:
            topics = collect_topics(json.loads(trends_f.read_text()))
    ...
    archive_index(days)
    print(f"rendered {day}: {len(tweets)} tweets, {len(topics)} topics → docs/{day}/index.html")
    return 0
